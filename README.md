...
Coco 是一家香港的多市场广告投放公司。运营人员原本需要为每个市场手写大量广告文案，
还要逐条做合规检查、翻译、SEO 关键词嵌入——既慢又容易踩平台政策红线。
...
| 🧬 **三层差异化** | 文本去重 → 语义向量去重（阈值 0.60）→ 创意角度拆分，保证同组文案不雷同 |
...
flowchart TD
    ...
    GEN --> D1[文本去重]
    D1 --> D2[语义向量去重<br/>SemanticDiversityChecker]
    D2 --> P[逐候选流水线 · 并发]
