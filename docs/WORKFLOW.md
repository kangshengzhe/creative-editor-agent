...
这份文档帮你从零理解：**一个请求从浏览器点「生成」开始，到拿到 15 条标题，中间到底发生了什么。**
...
flowchart TD
    ...
    O --> TC[算出目标数量 target_count<br/>标题=15 描述=10 CTA=5]
    ...
    TDEDUP --> SEM[语义去重 SemanticDiversityChecker<br/>嵌入相似度 > 0.60 判为近义重复并丢弃]
    SEM --> PIPE[逐条过流水线]
    ...
    MULT --> TRUNC[截断到目标数量<br/>18条择优取前15]
    TRUNC --> OUT[输出 AB_Ranking<br/>含15条标题+评分+翻译]
    OUT --> FE[前端展示结果]
```
...
| **目标数量 target_count** | 这次要凑够多少条：标题15、描述10、CTA5（业务方定的） |
| **文本去重** | 拦"一模一样"的重复 |
| **语义去重** | 拦"换了说法但意思一样"的重复（靠 AI 嵌入模型算相似度，>0.60 就丢） |
| **本地化** | 把英文文案翻成目标市场语言（菲律宾语、泰语等），或直接用母语生成 |
| **合规检查** | 拦违禁词：赌博保证、医疗承诺、虚假紧迫感等 |
| **综合评分** | 合规分 + 关键词覆盖 + CTA强度 加权算出的总分，用来排序 |
| **截断** | 生成可能超量（如18条），最后只保留分数最高的15条交付 |
...
flowchart TD
    ...
    D2 -->|嵌入相似度>0.60<br/>去掉近义重复| D3[第3道 角度生成]
    D3 -->|每条来自不同卖点角度<br/>保证结构性差异| R[剩下的就是互相差异化的候选]
```

- **第1道·文本去重**：`你好` 和 `你好` → 拦掉
- **第2道·语义去重**：`即时充值你的游戏钱包` 和 `充值你的游戏钱包，即时到账`（相似度0.92）→ 拦掉
- **第3道·角度生成**：强制从"省钱/快速/安全/信任…"等不同角度各生成，从源头上拉开差异

> 注意：语义去重依赖 `sentence-transformers` 模型，server 启动时会预加载（日志显示 `server.semantic_diversity_ready` 表示已就绪）。
...
```
浏览器填表 → server 收请求 → 校验 → 编排器算目标数量(15/10/5)
